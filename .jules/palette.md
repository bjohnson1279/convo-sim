...
## 2024-11-20 - Disabled Button Tooltips for Screen Readers\n**Learning:** When replacing a native `disabled` attribute with `aria-disabled` on interactive elements to allow screen reader focus (e.g., to read a `title` explaining why it's disabled), the element remains clickable.\n**Action:** You must always manually block the corresponding action on the backend (such as adding a guard clause to the LiveView `handle_event`) because `aria-disabled` does not natively prevent click events.
